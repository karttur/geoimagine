...
This post goes through creating a complete virtual Python environment in <span class='app'>Anaconda</span> for Karttur´s GeoImagine Framework. To learn the basics of setting up virtual environment in Anaconda, the [blog on setting up the Spatial Data Integrated Development Environment (SPIDE)](https://karttur.github.io/setup-ide/) includes a post on [Conda virtual environments](https://karttur.github.io/setup-ide/setup-ide/conda-environ/).  

# Prerequisites

Conda must be installed as described in the post on [Install Anaconda](https://karttur.github.io/setup-ide/setup-ide/conda-environ/) that is part of the the [blog on setting up the Spatial Data Integrated Development Environment (SPIDE)](https://karttur.github.io/setup-ide/).

# Conda virtual environments

A Python virtual environment is a stand-alone but completely copy of a specific Python version including site-packages (libraries). With a virtual environment you can create, and experiment with, different projects while avoiding problems with different dependencies and version requirements. The conda command is the preferred interface for managing installations and virtual environments with the Anaconda Python distribution.

To continue following this post you need to create a Python virtual environment. Go back to the post on [Conda virtual environments](https://karttur.github.io/setup-ide/setup-ide/conda-environ/) and create a new virtual environment using the default packages defined in that post. In the reamining of this post the new environment will be called 'geoimagine002'.
