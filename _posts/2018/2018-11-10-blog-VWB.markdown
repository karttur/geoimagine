...
As explained in the post on [Running processes](../setup-run/) you can create a project module for running specific tasks. The module below links to a single file (<span class='file'>VWB_TRMM-vs-FAOrefevap.txt</span>), that in turn links to a set of xml files. Running the module below will run all processes in the listed xml files.
